...
**Lógica obrigatória, dentro de `runTransaction`:**

1. `tx.get` em `idempotency/{key}` → se existe, devolver a resposta armazenada
   com `Idempotent-Replay: true`
2. `tx.get(cardRef)` — o Admin SDK trava o documento até o commit
3. Se `stampsCount >= stampsRequired` → `409 CARD_FULL`
4. Se `lastStampAt` há menos de 3 min → `409 STAMP_TOO_SOON`
5. `tx.create` do documento em `stamps`
6. `tx.update` no cartão: `stampsCount: increment(1)`, `lastStampAt`, `updatedAt`
7. `tx.update` nos contadores (`counters/totals`, `counters/daily_{data}`)
8. `tx.create` em `idempotency/{key}`
9. Commit
10. **Fora da transação:** atualizar os passes (Apple + Google)

Código de referência em `03-modelo-de-dados.md`, seção Transações.
...
**Lógica, em transação:**
1. `tx.get(cardRef)`
2. Se `stampsCount < stampsRequired` → `409 CARD_NOT_FULL`
3. `tx.create` em `redemptions/{cardId}_{cycle}` — o ID determinístico é o que
   impede resgate duplo no mesmo ciclo
4. `tx.update` no cartão: `stampsCount: 0`, `cycle: increment(1)`
...
**Streaming obrigatório.** Paginar a coleção e escrever na resposta conforme
lê — não carregar todos os clientes em memória. Firestore não tem cursor de
servidor como o Postgres; usar `startAfter` com o último documento.
...
**Processar em lotes de 400 documentos** — Firestore limita batches a 500.
Retornar `202` imediatamente e processar em background.
